...
axes.errorbar(desiLRG[0, 0], desiLRG[0, 1], 
              yerr=np.sqrt(np.diag(desiLRGcov))[0], markersize=3.5,
              marker='o', c='C0', label='DESI LRG ' + r'$D_A/r_s$')
axes.errorbar(desiLRG[1, 0], desiLRG[1, 1], 
              yerr=np.sqrt(np.diag(desiLRGcov))[1], markersize=3.5,
              marker='o', c='C1', label='DESI LRG ' + r'$D_H/r_s$')

axes.errorbar(desiLRGELG[0, 0], desiLRGELG[0, 1], 
              yerr=np.sqrt(np.diag(desiLRGELGcov))[0], markersize=3.5,
              marker='^', c='C0', label='DESI LRG+ELG ' + r'$D_A/r_s$')
axes.errorbar(desiLRGELG[1, 0], desiLRGELG[1, 1], 
              yerr=np.sqrt(np.diag(desiLRGELGcov))[1], markersize=3.5,
              marker='^', c='C1', label='DESI LRG+ELG ' + r'$D_H/r_s$')

axes.errorbar(desiELG[0, 0], desiELG[0, 1], 
              yerr=np.sqrt(np.diag(desiELGcov))[0], markersize=3.5,
              marker='x', c='C0', label='DESI ELG ' + r'$D_A/r_s$')
axes.errorbar(desiELG[1, 0], desiELG[1, 1], 
              yerr=np.sqrt(np.diag(desiELGcov))[1], markersize=3.5,
              marker='x', c='C1', label='DESI ELG ' + r'$D_H/r_s$')

axes.errorbar(sdss[0, 0], sdss[0, 1], 
              yerr=np.sqrt(np.diag(sdsscov))[0], markersize=3.5,
              marker='*', c='C0', label='SDSS LRG ' + r'$D_A/r_s$')
axes.errorbar(sdss[1, 0], sdss[1, 1], 
              yerr=np.sqrt(np.diag(sdsscov))[1], markersize=3.5,
                marker='*', c='C1', label='SDSS LRG ' + r'$D_H/r_s$')
axes.errorbar(sdss[2, 0], sdss[2, 1], markersize=3.5,
              yerr=np.sqrt(np.diag(sdsscov))[2], marker='*', c='C0')
axes.errorbar(sdss[3, 0], sdss[3, 1], markersize=3.5,
              yerr=np.sqrt(np.diag(sdsscov))[3], marker='*', c='C1')

plt.xlabel(r'$z$')
plt.ylabel(r'$D/r_s$')
plt.legend(loc='lower left', bbox_to_anchor=(-.3, 1.05), ncols=2)
#plt.tight_layout()
