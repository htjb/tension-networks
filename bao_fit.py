...
def likelihood(theta):
    try:
        ...
        L = (datad12 - d12[:, 1]).T @ np.linalg.inv(d12cov) @ (datad12 - d12[:, 1])
        L += (datad16 - d16[:, 1]).T @ np.linalg.inv(d16cov) @ (datad16 - d16[:, 1])
...
run_poly(prior, likelihood, file, RESUME = False, nDims=6)